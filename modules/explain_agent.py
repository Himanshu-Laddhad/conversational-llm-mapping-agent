...
Supports any provider configured in llm_client.PROVIDERS (OpenAI, Groq,
NVIDIA NIM, Anthropic). The default provider is OpenAI.
"""
...
def explain(
    ...
    Args:
        ...
        model:       LLM model identifier. Falls back to provider default.
        provider:    LLM provider — one of "openai", "groq", "nvidia_nim", etc.
        xslt_index:  Optional pre-built XSLT tool-calling index for large files.
    ...
    Example::
        ...
        from modules.explain_agent import explain

        ingested = ingest_file(file_path="data/810_NordStrom_Xslt_11-08-2023.xml")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("        python modules/explain_agent.py [file_path]")
        sys.exit(1)

    print("\n" + "=" * 80)
    print("  EXPLAIN AGENT — Test Harness")
    print("=" * 80 + "\n")
    ...
    if len(sys.argv) < 2:
        ...
        if not candidates:
            ...
            print("        python modules/explain_agent.py path/to/file\n")
            sys.exit(1)

        file_path = str(candidates[0])
        print(f"[DEMO] Auto-selected: {candidates[0].name}\n")
        print(f"       Usage: python modules/explain_agent.py <path_to_file>\n")
    else:
